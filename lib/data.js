...
export function mapPackageRow(p, driversById, evidenceByPkg) {
  ...
  return {
    ...
    lat: p.lat,
    lon: p.lon,
    locationStatus: p.location_status || ((p.lat != null && p.lon != null) ? 'confirmed' : 'unprocessed'),
    locationConfidence: p.location_confidence,
    locationProvider: p.location_provider,
...
export async function lookupKnownLocation(address) {
  ...
  return {
    lat: data.lat,
    lon: data.lon,
    display: data.address_label,
    confidence: 100,
    provider: 'ubicacion-confirmada',
    needsReview: false,
  };
}
...
export async function geocodeAddress(address) {
  ...
  if (data.providerErrors?.length) {
    // No interrumpe el flujo, pero deja rastro en la consola del navegador
    // para diagnosticar por qué un proveedor (ej. Google) no está aportando nada.
    console.warn('Geocodificación — errores de proveedor para "' + address + '":', data.providerErrors);
  }
  return data.result ? { ...data.result, needsReview: data.needsReview } : null;
}
...
/* ---------- Autocompletado de direcciones (Google Places) ----------
   El `sessionToken` agrupa una búsqueda completa (escribir + elegir) en una
   sola sesión facturable de Google, mucho más barato que cobrar cada letra
   tecleada como una solicitud aparte. Genera uno nuevo por cada búsqueda. */
export async function autocompleteAddress(input, sessionToken) {
  if (!input?.trim()) return [];
  const res = await fetch(`/api/places-autocomplete?input=${encodeURIComponent(input)}&sessiontoken=${encodeURIComponent(sessionToken)}`);
  if (!res.ok) return [];
  const data = await res.json();
...
export async function getPlaceDetails(placeId, sessionToken) {
  const res = await fetch(`/api/places-details?placeId=${encodeURIComponent(placeId)}&sessiontoken=${encodeURIComponent(sessionToken)}`);
  if (!res.ok) return null;
  const data = await res.json();
